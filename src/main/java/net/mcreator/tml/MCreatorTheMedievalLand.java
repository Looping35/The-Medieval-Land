...
public class MCreatorTheMedievalLand extends Elementstml.ModElement {
	@ObjectHolder("tml:themedievalland")
	public static final ModDimension dimension = null;
	@ObjectHolder("tml:themedievalland")
	public static final Item block = null;
	@ObjectHolder("tml:themedievalland_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;

	public MCreatorTheMedievalLand(Elementstml instance) {
		super(instance, 12);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("themedievalland"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("tml:themedievalland")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("tml:themedievalland"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("tml:themedievalland"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("tml:medievalplain")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new ModTrigger().setRegistryName("themedievalland"));
	}

	public static class ModTrigger extends Item {
		public ModTrigger() {
			super(new Item.Properties().group(MCreatorTheMedievalLandTAB.tab).maxDamage(64));
		}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("themedievalland_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == MCreatorStrangeLog.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == MCreatorStrangeLog.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == MCreatorStrangeLog.block.getDefaultState().getBlock())) {
								break label56;
							}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos, flag ? MCreatorStrangeLog.block.getDefaultState().getBlock()
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world
								.setBlockState(blockpos$mutableblockpos, MCreatorStrangeLog.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.556862745098, 0.592156862745, 0.654901960784);
		}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return MCreatorMedievalGrass.block.getDefaultState();
				}
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(MCreatorMedievalGrass.block.getDefaultState().getBlock(), biome.getSurfaceBuilder()
